...
public class Main {
    ...
    private static Boolean mayShoot = false;
    private static Boolean mayMove = false;
    private static Boolean mayMoveCheck = true;

    public void main(String[] args) throws InterruptedException, UnsupportedAudioFileException, IOException, LineUnavailableException {
        ...
        while(true) {
            if(GameHandler.gameRunning) {
                ...
                if(!Spaceship.dead && mayMove){
            		...
            		// Shooting
            		if(keys.contains(KeyEvent.VK_SPACE) || keys.contains(KeyEvent.VK_W) || keys.contains(KeyEvent.VK_UP)) {
            			if(mayShoot) {
            				Spaceship.shoot();
            				mayShoot = false;
